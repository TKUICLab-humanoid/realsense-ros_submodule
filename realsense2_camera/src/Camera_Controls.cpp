...
rs2::sensor Camera_control::get_a_sensor_from_a_device()
{
    rs2::context ctx; 
    rs2::device_list devices = ctx.query_devices();
    rs2::device selected_device = devices[0];
    std::vector<rs2::sensor> sensors = selected_device.query_sensors();
    uint32_t selected_sensor_index = 1; //RGB camera
...
int main(int argc, char **argv)
{
    ...
    while (nh.ok())
    {
        ...
        camera_control.sensor = camera_control.get_a_sensor_from_a_device();
        camera_control.get_sensor_all_option_value();
        ros::spinOnce();
        loop_rate.sleep();
    }
    return 0;
